module CrowdFlow::main {
    ...
    use sui::table::{Self, Table, add, new as new_table, contains, borrow};
    use sui::math;
    use std::ascii;
    use std::type_name::{Self, TypeName};
    use oracles::oracle::get_sui_price;
    use supra_holder::SupraSValueFeed::OracleHolder;
    use fungible_tokens::token::{Self as SupraToken};
    ...
    public struct Fundraiser has key, store {
        id: UID,
        name: vector<u8>,
        ...
        target: u64,
        raised: Balance<SUI>,
        milestones: Table<u64, bool>,
        token_name: vector<u8>,
        token_icon: vector<u8>,
        token_supply: Supply<TOKEN>,
        token_id: UID,
        token_treasury: coin::TreasuryCap<TOKEN>,
    }
    ...
    public entry fun create_fundraiser(target: u64, milestone_count: u64, tokenName: vector<u8>, symbol: vector<u8>, name: vector<u8>, images: vector<vector<u8>>, categories: vector<vector<u8>>, description: vector<u8>, region: vector<u8>, ctx: &mut TxContext) {
        let fund_uid = object::new(ctx);
        let fund_idd = object::uid_to_inner(&fund_uid);
        let mut milestone_table = table::new(ctx);
        let mut i = 0;

        loop {
            i = i + 1;
            if (i > milestone_count) break;
            table::add(&mut milestone_table, i, false);
        };

        // Create and deploy the fundraising token
        let (token_treasury, token_metadata) = SupraToken::create_currency(TOKEN {}, tokenName, symbol, ctx);
        let token_id = object::new(ctx);
        let token_supply = balance::create_supply(TOKEN {});
        ...
        transfer::public_freeze_object(token_metadata);

        transfer::share_object(fund);
    }

    public entry fun donate(oracle_holder: &OracleHolder, fund: &mut Fundraiser, amount: &mut Coin<SUI>, liquidity_pool_address: address, ctx: &mut TxContext) {
        let amount_donated: u64 = coin::value(amount);
        let half_amount = amount_donated / 2;

        // Split the donation: half goes to the fundraiser, half to the liquidity pool
        let fund_coin = coin::split(amount, half_amount, ctx);
        let fund_balance: Balance<SUI> = coin::into_balance(fund_coin);
        balance::join(&mut fund.raised, fund_balance);

        // Mint fundraiser tokens
        let token_amount = amount_donated / 2;
        SupraToken::mint(&mut fund.token_treasury, token_amount, tx_context::sender(ctx), ctx);


        // Mint tokens and transfer directly to the liquidity pool
        SupraToken::mint(&mut fund.token_treasury, half_amount, liquidity_pool_address, ctx);
        ...
        // Create and transfer the donation receipt
    ...
    public entry fun withdraw_funds(cap: &FundOwnerCap, fund: &mut Fundraiser, ctx: &mut TxContext) {
    ...
    public entry fun achieve_milestone(cap: &FundOwnerCap, fund: &mut Fundraiser, milestone_index: u64, ctx: &mut TxContext) {
        assert!(&cap.fund_id == object::uid_as_inner(&fund.id), EnotFundOwner);
        assert!(table::contains(&fund.milestones, milestone_index) == false, ENotMilestone);

        table::add(&mut fund.milestones, milestone_index, true);
        event::emit(MilestoneReached { milestone_index });

        // Adjust liquidity based on milestone
        // This is a placeholder for the actual liquidity adjustment logic
        // which would depend on the integration with the AMM
    }

    public fun get_target(fund: &Fundraiser): u64 {
        fund.target
    }

    public fun get_raised(fund: &Fundraiser): u64 {
    ...
    public entry fun create_pool_with_coins_and_transfer_lp_to_sender<A, B>(factory: &mut Factory, init_a: Coin<A>, init_b: Coin<B>, ctx: &mut TxContext) {
        let lp_balance = create_pool(factory, coin::into_balance(init_a), coin::into_balance(init_b), ctx);
        transfer::public_transfer(coin::from_balance(lp_balance, ctx), tx_context::sender(ctx));
    }
    ...
    public entry fun swap_a_for_b_with_coin_and_transfer_to_sender<A, B>(pool: &mut Pool<A, B>, input: Coin<A>, min_out: u64, ctx: &mut TxContext) {
        let b_out = swap_a_for_b(pool, coin::into_balance(input), min_out);
        transfer::public_transfer(coin::from_balance(b_out, ctx), tx_context::sender(ctx));
    }
    ...
    public entry fun swap_b_for_a_with_coin_and_transfer_to_sender<A, B>(pool: &mut Pool<A, B>, input: Coin<B>, min_out: u64, ctx: &mut TxContext) {
        let a_out = swap_b_for_a(pool, coin::into_balance(input), min_out);
        transfer::public_transfer(coin::from_balance(a_out, ctx), tx_context::sender(ctx));
    }
